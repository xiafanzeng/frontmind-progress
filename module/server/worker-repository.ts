...
import {createProgressRepository} from './monitoring-repository.js';
import type {ProgressDatabase as Database,ProgressTransaction as Transaction,ProgressWorkerCore} from './persistence-core.js';
import type {RunCompetitorMetric,RunModelMetric} from '../schema/index.js';

export function createProgressWorkerRepository(core:ProgressWorkerCore) {
 const {attempts,attemptMoneySettlements,attemptPriceSnapshots,attemptResults,dailyRunAggregates,jobs,monitors,monitorVersions,platformCatalog,projectBrandVersions,projects,providerCosts,providerDispatchDays,providerDispatchSlots,providerObservations,providerReconciliationState,providerRegions,providerSubmissionGate,providerTaskTombstones,resultDiscoveredSources,resultMedia,resultRevisions,resultSources,runMetricDomains,runMetrics,runs,scheduleOccurrences,workerHeartbeats} = core.tables;
 const {settleAttemptMoney,syncDashboardMonitoringAccountStates,officialPricingClassForKnownProvider}=core;
 const {computeNextRunAt,pollAttemptJobDedupeKey}=createProgressRepository(core);
...
class DrizzleWorkerRepository {
  readonly monitoring: ReturnType<ProgressWorkerCore["monitoringRepository"]>;

  constructor(
    readonly db: Database,
    readonly callbackUrl?: string,
  ) {
    this.monitoring = core.monitoringRepository(db);
  }
    ...
    await this.db.transaction(async (tx) => {
      let lockedAttempt:
        | Pick<
            ...
            "id" | "runId" | "status" | "errorCode" | "providerTaskId"
          >
        | undefined;
      ...
      if (
        ...
        if (!terminalAttempt(lockedAttempt.status)) {
          const unresolvedSubmission =
            lockedAttempt.errorCode === "SUBMISSION_UNKNOWN" &&
            !lockedAttempt.providerTaskId;
          ...
          await tx
            ...
            .set({
              ...
              errorCode: unresolvedSubmission
                ? "SUBMISSION_UNKNOWN"
                : "JOB_DEAD_LETTER",
              errorMessage: error,
            })
  ...
  > {
    ...
    return this.db.transaction(async (tx) => {
      ...
      if (attempt.providerTaskId) return { status: "terminal" as const };
      if (
        run.cancelRequestedAt &&
        attempt.status === "queued" &&
        attempt.errorCode !== "SUBMISSION_UNKNOWN"
      ) {
        await settleAttemptMoney(tx, {
          attemptId: attempt.id,
          settlement: "released",
          settledAt: input.now,
          reason: "Cancelled before provider accepted submission",
        });
        await tx
          .update(attempts)
          .set({
        ...
        await refreshRunState(tx, run, input.now);
        return { status: "terminal" as const };
      }
      if (
  ...
  async recordAttemptSubmissionRejection(input: {
    attemptId: string;
    retryable: boolean;
    retriesExhausted: boolean;
    reason: string;
    observedAt: Date;
  }): Promise<"retry" | "terminal" | "unknown"> {
    const [located] = await this.db
      .select({ runId: attempts.runId })
      .from(attempts)
      .where(eq(attempts.id, input.attemptId))
      .limit(1);
    if (!located) return "terminal";
    return this.db.transaction(async (tx) => {
      const [run] = await tx
        .select()
        .from(runs)
        .where(eq(runs.id, located.runId))
        .for("update")
        .limit(1);
      ...
      const [attempt] = await tx
        ...
        .for("update")
        .limit(1);
      if (!attempt || terminalAttempt(attempt.status) || attempt.providerTaskId)
        return "terminal";
      // A later rejection cannot disprove acceptance of an earlier timed-out
      // request. The existing error marker survives reacquiring a permit.
      if (
        attempt.errorCode === "SUBMISSION_UNKNOWN" ||
      ...
      const retry =
        !run.cancelRequestedAt && input.retryable && !input.retriesExhausted;
      const status = run.cancelRequestedAt
        ? ("cancelled_before_submit" as const)
        : retry
          ? ("queued" as const)
          : ("failed" as const);
      if (!retry) {
        await settleAttemptMoney(tx, {
          ...
          reason: run.cancelRequestedAt
            ? "Cancelled after provider rejected submission"
            : "Provider submission was rejected",
        });
      }
      await tx
        .update(attempts)
        .set({
          status,
          nextPollAt: null,
          terminalAt: retry ? null : input.observedAt,
          errorCode: run.cancelRequestedAt
            ? "CANCELLED_BEFORE_SUBMIT"
            : retry
              ? "PROVIDER_TEMPORARY_REJECTION"
              : "PROVIDER_SUBMISSION_REJECTED",
          errorMessage: run.cancelRequestedAt
            ? "已取消，本次未取得平台回答"
            : retry
              ? "平台暂时未能接收请求，正在重试"
              : "平台未能接收本次请求，请稍后重新运行",
      ...
      await tx.insert(providerObservations).values({
        ...
        attemptId: attempt.id,
        type: "submission",
        status: retry ? "temporary_provider_rejection" : "submission_rejected",
        payload: {
          reason: truncate(input.reason, 1_000),
    ...
    const rows = await this.db
      ...
      .where(
        and(
          ...
          core.activeOwnerCondition(monitors.ownerId),
  ...
  async listPurgeCandidates(through: Date, limit: number) {
    await core.cleanupIdentityRetention(this.db, through);
    ...
    await this.db.transaction(async (tx) => {
      ...
      if (runIds.length > 0) {
        ...
        if (attemptIds.length > 0) {
          await core.detachAttemptFunds(tx, attemptIds);
          await tx
            .update(providerCosts)
            .set({ attemptId: null })
        ...
        await core.detachRunFunds(tx, runIds);
        await tx
        ...
        await tx
          ...
          .onDuplicateKeyUpdate({
            set: {
              ...
              acceptanceRequired: true,
              acceptanceFingerprint,
              // Screenshot capability follows the vendor submit-task contract
              // (0/1/2 accepted for every model), never local acceptance.
              supportsScreenshot: moliEffectiveScreenshotSupport(),
              ...(evidenceChanged
                ? {
                    verified: false,
...
function countsTowardSubmittedAttempts(status: AttemptStatus) {
...
function explicitCitationReferences(
...
return {DrizzleWorkerRepository,countsTowardSubmittedAttempts,explicitCitationReferences};
}
