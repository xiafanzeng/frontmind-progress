...
export class WorkerProcessor {
  ...
  constructor(
    ...
    private readonly infrastructure: ProgressInfrastructure,
  ...
  private async submitAttempt(
    attemptId: string,
    job: Pick<WorkerJob, "attemptCount" | "maxAttempts">,
    signal?: AbortSignal,
  ): Promise<void> {
    const attempt =
    ...
    const consumerTaskId =
      attempt.consumerTaskId ?? this.infrastructure.createConsumerTaskId(attempt.attemptId);
    ...
    let submitted: { taskId: string; subTaskId?: string };
    try {
      submitted = await this.dependencies.provider.submitSingleAttempt(
        {
          attemptId,
          consumerTaskId,
      ...
      if (this.infrastructure.classifyProviderError(error).submissionUnknown) {
      ...
      const outcome =
        await this.dependencies.repository.recordAttemptSubmissionRejection({
          attemptId,
          retryable: this.infrastructure.classifyProviderError(error).retryable,
          retriesExhausted: job.attemptCount + 1 >= job.maxAttempts,
          reason: `${safeErrorSummary(error)} (${this.infrastructure.classifyProviderError(error).code ?? "submission_rejected"})`,
          observedAt: this.now(),
        });
      if (outcome === "unknown") {
        throw new DeferJobError(
          "An earlier submission still requires confirmation",
          new Date(this.now().valueOf() + PROVIDER_RECOVERY_DELAY_MS),
        );
      }
      if (outcome === "retry") {
        // Use the existing durable retry budget and backoff. A rejected POST
        // is not an indefinitely pending provider submission.
        throw error;
      }
      return;
    }
    ...
    try {
      ...
      // The POST succeeded. A persistence failure is not a rejected submission.
      await this.dependencies.repository.markAttemptSubmissionUnknown({
        attemptId,
        consumerTaskId,
        observedAt: this.now(),
      ...
      throw new DeferJobError(
        "Accepted submission is awaiting local confirmation",
        new Date(this.now().valueOf() + PROVIDER_RECOVERY_DELAY_MS),
      );
    }
  }
  ...
  private async fetchResult(
    ...
    const compressedRaw = this.infrastructure.compressJsonPayload(result.raw);
    const rawObjectKey = this.infrastructure.createRawResultObjectKey({
  ...
  private async archiveMedia(
    ...
    try {
      const fetched = await (
        this.dependencies.mediaFetcher ?? this.infrastructure.fetchRemoteMedia
      )(candidate.sourceUrl, {
        maxBytes: this.mediaMaxBytes,
        signal,
      });
      const keys = this.infrastructure.createArchivedMediaKeys({
      ...
      const derivatives = await (
        this.dependencies.imageTransformer ?? this.infrastructure.createImageDerivatives
      )(fetched.body);
      await Promise.all([
      ...
      const terminal = this.infrastructure.isRejectedMedia(error);
