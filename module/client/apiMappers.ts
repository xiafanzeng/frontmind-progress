...
export function mapProviderModel(
  ...
  return {
    ...
    reasoningSupport: value.reasoningSupport,
    capabilities: {
      reasoning: value.supportsReasoning,
      // The server derives effective screenshot capability from the vendor
      // submit contract (0/1/2 accepted for every model); acceptance records
      // stay diagnostics-only and never disable the options here.
      screenshot: value.supportsScreenshot,
      screenshotMention: value.supportsScreenshot,
      screenshotAll: value.supportsScreenshot,
...
export function monitorDetailToInput(
  ...
  return {
    ...
    brandAliases: output.version.brandAliases || [],
...
export function monitorInputToConfiguration(
  ...
  return {
    name: value.name,
    brandAliases:
      value.brandAliases && value.brandAliases.length
        ? value.brandAliases
        : project.brandAliases,
    competitors: value.competitors,
    questions: value.questions,
    questionCategories: value.questionCategories,
    platforms: value.platforms.map((selection) => {
      ...
      return {
        ...
        mode: selection.mode,
        // Pass the chosen screenshot policy through as-is (0/1/2); the
        // server applies the same vendor-contract validation everywhere.
        screenshot: asScreenshot(selection.screenshot),
        regionCode:
...
export function mapMonitorDetailSummary(
  ...
  return {
    ...
    scheduleType: value.monitor.scheduleType,
    scheduleLabel: scheduleLabel(
...
export function mapRunDetail(output: Outputs["runs"]["get"]): MonitorRun {
  ...
  const attempts: RunAttempt[] = output.attempts.map(({ attempt, result }) => {
    ...
    return {
      ...
      questionCategory:
        output.configuration.questions.find(
          (question) => question.ordinal === attempt.monitorQuestionOrdinal,
      ...
      screenshotBrandMentioned: result?.brandMentioned ?? null,
      ...
      capturedAt: result ? iso(result.updatedAt) : undefined,
      submittedAt: iso(attempt.submittedAt),
      terminalAt: iso(attempt.terminalAt),
