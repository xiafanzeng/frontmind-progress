...
export default function RunTrendAnalysis({
  runs,
  version,
  showIndustryMetrics: allowIndustryMetrics = true,
  ...
  if (allComparable.length < 2) {
    return (
      <div className="analysis-placeholder trend-empty">
        ...
        <span>
          至少两个同版本已完成运行后显示有效回答、
          {showIndustryMetrics ? "品牌提及率、" : ""}引用和模型表现趋势。
        </span>
      </div>
    );
  }
      ...
      <div className="trend-run-list">
        {comparable.map((candidate) => (
          <article key={candidate.id}>
            ...
            <small>
              {showIndustryMetrics ? "品牌提及率 · " : ""}
              {candidate.metrics.citations} 条引用 · 有效回答{" "}
              {candidate.metrics.effectiveAnswers ??
                candidate.metrics.completed}
              个
            </small>
            <small>
              成功回答 {candidate.metrics.completed} · 失败{" "}
            ...
            <div
              ...
              style={
                showIndustryMetrics
                  ? undefined
                  : {
                      gridTemplateColumns: "130px minmax(190px, 1fr) 80px 70px",
                      minWidth: 530,
                    }
              }
            >
              ...
              <div
                ...
                style={
                  showIndustryMetrics
                    ...
                    : {
                        gridTemplateColumns:
                          "130px minmax(190px, 1fr) 80px 70px",
                        minWidth: 530,
                      }
                }
              >
