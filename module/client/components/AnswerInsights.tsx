...
> = {
  positive: { label: "正向", shortLabel: "正", color: "#17a980" },
  neutral: { label: "中性", shortLabel: "中", color: "#8b9bb1" },
  negative: { label: "负向", shortLabel: "负", color: "#eb4f6f" },
  unknown: { label: "未判定", shortLabel: "未", color: "#d4d9e2" },
};
        ...
        <Dialog.Content className="insight-dialog-content">
          ...
          <div className="insight-method-grid">
            <section>
              <strong>标准</strong>
              <p>
                只统计已完成且正文非空的回答。正向、中性、负向表示平台对品牌的表达倾向；没有明确结论时归入“未判定”。
              </p>
            </section>
            <section>
              <strong>含义</strong>
            ...
            <section>
              <strong>如何使用</strong>
              <p>
                正向、中性、负向、未判定四类均以全部有效回答为分母。评价词保留平台提供的判断和原文依据。
              </p>
            </section>
          </div>
        </Dialog.Content>
        ...
        <Dialog.Content className="insight-dialog-content keyword-context-dialog">
          <div className="insight-dialog-head">
            <div>
              ...
              <Dialog.Description>
                平台判断为{sentimentMeta[item.nature].label} · 涉及{" "}
                {item.answerCount}
                个有效回答
              </Dialog.Description>
            </div>
          ...
          {item.conflicted && (
            <p className="keyword-conflict-note" role="note">
              同一归一化关键词在不同回答中出现了不同情感分类；这里保留平台
              的原始分类，不合并推断。
            </p>
          )}
...
export function AnswerSentimentInsights({
  ...
  const positiveEnd = percentage(insight.counts.positive, total);
  const neutralEnd = positiveEnd + percentage(insight.counts.neutral, total);
  const negativeEnd = neutralEnd + percentage(insight.counts.negative, total);
  const donut = total
    ? `conic-gradient(${sentimentMeta.positive.color} 0 ${positiveEnd}%, ${sentimentMeta.neutral.color} ${positiveEnd}% ${neutralEnd}%, ${sentimentMeta.negative.color} ${neutralEnd}% ${negativeEnd}%, ${sentimentMeta.unknown.color} ${negativeEnd}% 100%)`
    : "#eef0f4";
        ...
        <article className="sentiment-summary-card">
          ...
          <div className="sentiment-donut-wrap">
            <div
              ...
              role="img"
              aria-label={`有效回答 ${total} 条，正向 ${insight.counts.positive} 条，中性 ${insight.counts.neutral} 条，负向 ${insight.counts.negative} 条，未判定 ${insight.counts.unknown} 条`}
            >
              <div>
                <strong>{total}</strong>
                <span>有效回答</span>
            ...
            <div className="sentiment-legend">
              {(
                ...
                <div key={nature}>
                  ...
                  <small>{percentage(insight.counts[nature], total)}%</small>
                </div>
              ))}
            </div>
          </div>
          ...
          <footer>
            正向、中性、负向与未判定均以 {total} 条有效回答为分母。
          </footer>
        </article>
        <article className="sentiment-method-card">
          <span>口径说明</span>
          <strong>品牌评价依据</strong>
          <p>
            情感与评价词均来自平台提供的品牌分析。缺少明确结论的回答显示为未判定。
          </p>
          <dl>
            <div>
              <dt>有效回答</dt>
...
export function IndustrySourceInsights({
  ...
  const citationEvidenceWarnings = [
    citationEvidence.legacy_assumed
      ? `${citationEvidence.legacy_assumed} 条回答暂无文内引用；相关参考可在“全部来源”中查看，不计入引用统计。`
      : "",
    citationEvidence.unavailable
      ? `${citationEvidence.unavailable} 条回答暂无文内引用，相关参考单独展示。`
      : "",
  ].filter(Boolean);
