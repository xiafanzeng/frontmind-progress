...
export function readMonitoringQuery(
  ...
  const questionCategory = KEYWORD_CATEGORY_OPTIONS.find(
    (item) => item.key === params.get("questionCategory"),
  )?.key;
  ...
  const filteredAttempts =
    attempts.filter(
      (attempt) =>
        ...
        (!questionCategory || attempt.questionCategory === questionCategory) &&
...
export function monitoringQueryString(state: MonitoringQueryState) {
  ...
  if (state.questionCategory)
    params.set("questionCategory", state.questionCategory);
...
export function writeMonitoringQuery(
  ...
  if (typeof window === "undefined") return;
  // Embedded reports share the host URL. Replace only this module's fields;
  // host routing (for example view=reports) and enterprise scope must survive.
  const params = new URLSearchParams(window.location.search);
  for (const key of [
    "project",
    "tab",
    ...
    "from",
    "to",
    ...projectScopedParameters,
  ])
    params.delete(key);
  for (const [key, value] of new URLSearchParams(monitoringQueryString(state)))
...
export function monitoringExportHref(
  ...
  if (!isMonitoringUuid(state.monitorId)) return;
  ...
  if (isMonitoringUuid(state.question)) {
    params.set("questionId", state.question);
  }
  if (isMonitoringUuid(state.model)) {
