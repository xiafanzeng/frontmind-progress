import type { KeywordCategoryKey } from "@frontmind/module-ui/keyword-categories";

export type MonitoringQuestionCategory = KeywordCategoryKey;
export type MonitoringQuestionSeed = {
  question: string;
  category?: MonitoringQuestionCategory | null;
};
...
/** Old payloads may confirm support, but a false flag cannot prove absence. */
export function modelReasoningSupport(model: ProviderModel): ReasoningSupport {
  return (
    model.reasoningSupport ??
    (model.capabilities.reasoning ? "supported" : "unknown")
  );
}
...
export type MonitorInput = {
  name: string;
  /** Monitor-scoped brand identity; first entry is the primary brand. */
  brandAliases: string[];
  competitors: CompetitorInput[];
  questions: string[];
  questionCategories?: Record<string, MonitoringQuestionCategory>;
...
export type RunAttempt = {
  ...
  questionId: string;
  question: string;
  questionCategory?: MonitoringQuestionCategory | null;
  ...
  screenshotBrandMentioned?: boolean | null;
  regionCode?: string;
  repetition: number;
